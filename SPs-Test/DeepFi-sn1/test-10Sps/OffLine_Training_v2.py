...
#random_point=list(np.array([2,5,8,10,14,18])-1)


if os.path.isdir(os.getcwd()+'/model'):
        shutil.rmtree(os.getcwd()+'/model')
os.mkdir(os.getcwd()+'/model')
for test_i in range(1,11):   #we must create a session for each of loc
    random_point = list(np.array([2, 4,6, 8, 10,12, 14,16,17, 19]) - 1)
